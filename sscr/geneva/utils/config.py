...
def parse_config():
    ...
    parser.add_argument(
        ...
        type=float,
        default=2e-3,
        help="Learning rate for image encoder and condition" "encoder. Default= 2e-3",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=4,
        help="Gradient clipping threshold for RNN and GRU." "Default: 4",
    )
    ...
    parser.add_argument(
        "--rnn_lr", type=float, default=0.0005, help="RNN optimizer learning rate"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Ratio of wrong:fake losses." "Default: 0.5",
    )
    # Strings
    ...
    parser.add_argument(
        ...
        type=str,
        default="relu",
        help="Activation function to use." "Options = [relu, leaky_relu, selu]",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="resblocks",
        help="Network Architecture to use. Two options are" "available {resblocks}",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="hinge",
        help="Loss function to use. Options:" "{classical, hinge} Default: hinge",
    )
    ...
    parser.add_argument(
        ...
        help="Gan type: recurrent. Options:" "[recurrent_gan]. Default: recurrent_gan",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="res_blocks",
        help="Building blocks of the image encoder." " Default: res_blocks",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Snapshot file to load model and optimizer" "state from",
    )
    ...
    parser.add_argument(
        "--rnn_optimizer", type=str, default="rmsprop", help="Optimizer to use for RNN"
    )

    parser.add_argument("--test_dataset", type=str, help="Test dataset path key.")

    parser.add_argument("--val_dataset", type=str, help="Validation dataset path key.")
    ...
    parser.add_argument(
        "-debug", action="store_true", help="Debugging flag.(e.g, Do not save weights)"
    )

    parser.add_argument(
        "-disc_sn",
        action="store_true",
        help="A flag that decides whether to use spectral norm" "in the discriminator",
    )

    parser.add_argument(
        "-generator_sn",
        action="store_true",
        help="A flag that decides whether to use" "spectral norm in the generator.",
    )

    parser.add_argument(
        "-generator_optim_image",
        action="store_true",
        help="A flag of whether to optimize the image encoder" "w.r.t the generator.",
    )

    parser.add_argument(
        "-inference_save_last_only",
        action="store_true",
    ...
    parser.add_argument(
        "-self_attention",
        action="store_true",
        help="A flag that decides whether to use" "self-attention layers.",
    )
