...
class InferenceRecurrentGAN:
    def __init__(self, cfg):
        ...
        self.generator = DataParallel(
            GeneratorFactory.create_instance(cfg), device_ids=[0]
        ).cuda()

        self.rnn = nn.DataParallel(
            nn.GRU(cfg.input_dim, cfg.hidden_dim, batch_first=False),
            dim=1,
            device_ids=[0],
        ).cuda()

        self.layer_norm = nn.DataParallel(
            nn.LayerNorm(cfg.hidden_dim), device_ids=[0]
        ).cuda()

        self.image_encoder = DataParallel(ImageEncoder(cfg), device_ids=[0]).cuda()

        self.condition_encoder = DataParallel(
            ConditionEncoder(cfg), device_ids=[0]
        ).cuda()

        self.sentence_encoder = nn.DataParallel(
            SentenceEncoder(cfg), device_ids=[0]
        ).cuda()
    ...
    def predict(self, batch):
        with torch.no_grad():
            ...
            prev_image = (
                torch.FloatTensor(batch["background"])
                .unsqueeze(0)
            ...
            for t in range(max_seq_len):
                ...
                if (not self.cfg.inference_save_last_only) or (
                    self.cfg.inference_save_last_only and t == max_seq_len - 1
                ):
        ...
        _save_predictions(
            generated_images, batch["turn"], scene_id, self.results_path, gt_images
        )
...
def _save_predictions(images, text, scene_id, results_path, gt_images):
    for i, scene in enumerate(scene_id):
        ...
        for t in range(len(images)):
            ...
            cv2.imwrite(
                os.path.join(results_path, str(scene), "{}_{}.png".format(t, query)),
                image,
            )
            cv2.imwrite(
                os.path.join(
                    results_path, str(scene) + "_gt", "{}_{}.png".format(t, query)
                ),
                gt_image,
            )
