...
class Trainer:
    def __init__(self, cfg):
        ...
        if glob.glob(img_path):
            raise Exception(
                "all directories with name train_images_* under "
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.cfg.epochs), ascii=True):
            ...
            for batch in self.dataloader:
                if (
                    iteration_counter >= 0
                    and iteration_counter % self.cfg.save_rate == 0
                ):

                    torch.cuda.empty_cache()
                    evaluator = Evaluator.factory(
                        self.cfg, self.visualizer, self.logger
                    )
                ...
                self.model.train_batch(
                    batch, epoch, iteration_counter, self.visualizer, self.logger
                )
    ...
    def train_with_ctr(self):
        ...
        for epoch in tqdm(range(self.cfg.epochs), ascii=True):
            ...
            for batch in self.dataloader:
                ...
                iteration_counter += 1
                self.model.train_batch_with_ctr(
                    batch, epoch, iteration_counter, self.visualizer, self.logger
                )
    ...
    def infer_ctr(self):
        ...
        for batch in dataloader:
            rec_out, loss = self.model.train_ctr(
                batch, -1, -1, self.visualizer, self.logger, is_eval=True, is_infer=True
            )
            ...
            for i in range(30):
                ...
                for j in range(rec_out.shape[1]):
                    print(
                        [glove_key[rec_out[i, j, k]] for k in range(rec_out.shape[2])]
                    )
                    print(
                        [
                            glove_key[
                                int(batch["turn_word"][i, j, k].detach().cpu().numpy())
                            ]
                            for k in range(rec_out.shape[2])
                        ]
                    )
                    print()
                    ...
                    TV.utils.save_image(
                        batch["image"][i, j].data,
                        "ins_result/%d/%d.png" % (i, j),
                        normalize=True,
                        range=(-1, 1),
                    )
        ...
        for batch in dataloader:
            ...
            for i in range(30):
                ...
                for j in range(rec_out.shape[1]):
                    F.write(
                        " ".join(
                            [
                                ...
                                for k in range(batch["turn_word"].shape[2])
                            ]
                        )
                    )
                    F.write("\n")
                    ...
                    TV.utils.save_image(
                        batch["image"][i, j].data,
                        "gen_result/%d/_%d.png" % (i, j),
                        normalize=True,
                        range=(-1, 1),
                    )
                    TV.utils.save_image(
                        torch.from_numpy(rec_out[i, j]).data,
                        "gen_result/%d/%d.png" % (i, j),
                        normalize=True,
                        range=(-1, 1),
                    )
